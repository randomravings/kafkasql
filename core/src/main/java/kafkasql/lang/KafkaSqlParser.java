...
public class KafkaSqlParser {
  ...
  public static ParseResult parseFiles(List<Path> files, ParseArgs args) {
    ...
    if (args.resolveIncludes()) {
      ...
      paths = ordered;  // ordered is already List<Path>, no conversion needed
    } else {
      paths = files;
    }
    ...
    for (Path f : paths) {
      ...
      try {
        ...
        diags.fatal(Range.NONE, "Error reading file: " + f.toAbsolutePath().toString() + ": " + e.getMessage());
      }
    }
  ...
  private static Ast parseText(ParseSource src, Diagnostics diags, ParseArgs args) {
    ...
    try {
      ...
      diags.fatal(range, "Parse error: " + e.getMessage());
      return Ast.EMPTY;
    }
    catch (AstBuildException e) {
      diags.fatal(e.range(), e.getMessage());
      return Ast.EMPTY;
    }
  }
